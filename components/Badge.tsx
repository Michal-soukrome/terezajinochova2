...
export function Badge({ variant, locale, className = "" }: BadgeProps) {
  if (variant === "premium") {
    return (
      <span
        className={`inline-flex items-center gap-1 px-3 py-1.5 bg-linear-to-r from-amber-100 to-amber-50 border border-amber-300 text-amber-900 rounded-full text-xs font-semibold uppercase tracking-wide ${className}`}
      >
